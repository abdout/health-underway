  ...
  try {
    ...
    if (name || fullname) {
      await db.user.update({
        ...
        data: {
          name: name || undefined,
          // fullname goes to doctor profile
        }
      });
    }
    ...
    await db.doctor.upsert({
      ...
      update: {
        ...doctorData,
        // fullname should be stored in doctor profile based on schema
        // Note: Need to add fullname to Doctor model if not already there
      },
      create: {
        userId: user.id,
        ...doctorData,
...
export async function deleteInformation() {
  try {
    ...
    // Delete doctor profile (this will cascade delete related data)
