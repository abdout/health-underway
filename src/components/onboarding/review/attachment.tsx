          ...
          {hasAttachments ? (
            <div className="space-y-4">
              ...
              {userData?.cv && (
                <div 
                  className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors"
                  onClick={() => openPdf(userData.cv, "CV", "cv.pdf")}
                >
              ...
              {userData?.portfolio && (
                <div 
                  className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors"
                  onClick={() => openPdf(userData.portfolio, "Portfolio", "portfolio.pdf")}
                >
                  <div className="w-10 h-10 bg-gray-100 rounded-md flex items-center justify-center">
                    <span className="text-xs">PDF</span>
                  </div>
              ...
              {userData?.additionalFile && (
                <div 
                  className="flex items-center gap-3 cursor-pointer hover:bg-gray-50 p-2 rounded-md transition-colors"
                  onClick={() => openPdf(userData.additionalFile, "Additional File", "additionalFile.pdf")}
                >
                  <div className="w-10 h-10 bg-gray-100 rounded-md flex items-center justify-center">
                    <span className="text-xs">PDF</span>
                  </div>
                  <span className="text-sm">Additional File</span>
                </div>
              )}
            </div>
          ) : (
